 ...
 * Return: Void
 */
void ft_check_chain(inf_t *inf, char *buf, size_t *p, size_t i, size_t len)
{
	size_t j = *p;
 ...
 */
int ft_replace_alias(inf_t *inf)
{
	int i;
	list_t *node;
	char *p;
	...
	for (i = 0; i < 10; i++)
	{
		node = ft_node_starts(inf->alias, inf->argv[0], '=');
		if (!node)
			return (0);
		free(inf->argv[0]);
		p = _strchr(node->str, '=');
...
int ft_replace_vars(inf_t *inf)
{
	...
	for (i = 0; inf->argv[i]; i++)
	{
		...
		{
			ft_replace_string(&(inf->argv[i]),
				_strdup(ft_convert_number(inf->status, 10, 0)));
			continue;
		}
		if (!_strcmp(inf->argv[i], "$$"))
		{
			ft_replace_string(&(inf->argv[i]),
				_strdup(ft_convert_number(getpid(), 10, 0)));
			continue;
		}
		node = ft_node_starts(inf->env, &inf->argv[i][1], '=');
		if (node)
		{
			ft_replace_string(&(inf->argv[i]),
		...
		ft_replace_string(&inf->argv[i], _strdup(""));
